 ...
 * Main Application JavaScript - localStorage version
 */
...
function addActionItem(taskId, inputElement) {
    ...
    saveTasks();
    renderTasks();
    showToast('세부 항목이 추가되었습니다.', 'success');
}
...
function deleteActionItem(taskId, actionId) {
    ...
    saveTasks();
    renderTasks();
    showToast('세부 항목이 삭제되었습니다.', 'success');
}
...
function updateSelectedAssigneesText() {
    ...
    if (textElement) {
        textElement.textContent = selected.length === 0 ? '담당자 선택' : selected.join(', ');
    }
}
            ...
            ${assignees.map((name, index) => `
                <div class="assignee-item">
                    ...
                    <div class="assignee-actions">
                        <button class="btn btn-sm btn-outline" onclick="editAssigneeName(${index})">✏️</button>
                        <button class="btn btn-sm btn-danger" onclick="deleteAssigneeByName(${index})">🗑️</button>
                    </div>
                </div>
            `).join('')}
        </div>
        <div class="add-assignee-form">
            <input type="text" id="newAssigneeName" placeholder="새 담당자 이름..." 
                   onkeypress="if(event.key === 'Enter') addNewAssignee()">
            <button class="btn btn-primary" onclick="addNewAssignee()">추가</button>
        </div>
    `;
...
function editAssigneeName(index) {
    const span = document.getElementById(`assignee-name-${index}`);
    const current = assignees[index];
    span.innerHTML = `<input type="text" value="${escapeHtml(current)}" style="width:120px;padding:4px 8px;border:1px solid var(--border-color);border-radius:4px;" onkeypress="if(event.key==='Enter')saveAssigneeEdit(${index},this.value)" onblur="saveAssigneeEdit(${index},this.value)" id="edit-input-${index}">`;
...
function saveAssigneeEdit(index, newName) {
    ...
    if (assignees.some((n, i) => i !== index && n === name)) { showToast('이미 존재합니다.', 'error'); openAssigneeManager(); return; }
    const oldName = assignees[index];
    assignees[index] = name;
    saveAssignees();
    tasks.forEach(t => {
        if (t.assignees) {
